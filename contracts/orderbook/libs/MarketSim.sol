...
library MarketSim {
    function selectTokens(address collection, uint256 scanLimit, uint8 density)
        internal
        pure
    ...
    function priceOf(address collection, uint256 tokenId) internal pure returns (uint256) {
        bytes32 h = keccak256(abi.encode("DMRKT_PRICE_V1", collection, tokenId));
